...
###
### source('C:/fasterRaster_benchmarking_distributed/fasterRaster_benchmarking/02_benchmarking_terra_loop.r')
###
### CONTENTS ###
### settings ###
### benchmark ###
...
say('#################')
	...
	sink(paste0(output_dir, tolower(demesne), '_terra_benchmark_k_start_', k_start, '.txt'), split = TRUE)
	say('BENCHMARK TERRA')
	say(date(), post = 1)
	...
	if (k_start > 1) {
		for (i in 1:(k_start - 1)) {
			prediction_rasts[[i]] <- rast(paste0(output_dir, 'prediction_k', prefix(i, 3), '.tif'))
		}
	}	
	...
	while (k <= n_folds) {
		...
		if (!model_multivar$converged | model_multivar$boundary) {
			...
			say('Model did not converge. Restarting model.')
			...
			fwrite(results, paste0(output_dir, tolower(demesne), '_terra_results_k_start_', k_start, '.csv'), row.names = FALSE)
	...
	start <- Sys.time()
	prediction_mean <- mean(predictions, filename = tempfile(fileext = '.tif'))
	stop <- Sys.time()
	...
	start <- Sys.time()
	prediction_sd <- stdev(predictions, pop = FALSE, filename = tempfile(fileext = '.tif'))
	stop <- Sys.time()
